...
import {
  Check, Crown, Star, Zap, AlertCircle, ExternalLink,
  ChevronDown, ChevronUp, CreditCard, ArrowLeft, X,
} from 'lucide-react';
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-sm mb-2">
        ...
        <span className={`font-semibold tabular-nums ${nearLimit ? 'text-[#FF8364]' : 'text-gray-500'}`}>
            ...
            <thead>
              <tr className="bg-gray-50">
                ...
                {cols.map((p) => (
                  ...
                  >
                    {p === currentPlan ? `${PLAN_META[p].label} ← you` : PLAN_META[p].label}
                  </th>
                ))}
              </tr>
            </thead>
...
function BillingContent() {
  ...
  // When user returns from checkout with ?success=true, immediately sync
  // subscription state from Lemon Squeezy — handles webhook delivery lag
  // and localhost environments where webhooks can't reach the backend.
  useEffect(() => {
    if (!success) return;
    setIsSyncing(true);
  ...
  }, []); // run once on mount when success param is present
                  ...
                  <span
                    ...
                    style={
                      status.subscription_status === 'active'
                        ? { backgroundColor: '#93DA97/20', color: '#2A6A2E', background: 'rgba(147,218,151,0.2)' }
                        : { backgroundColor: '#FACE68/20', color: '#7A5800', background: 'rgba(250,206,104,0.2)' }
                    }
                  >
                    {status.subscription_status}
                  </span>
          ...
          {status.subscription_canceled_at && status.plan_expires_at && (
            <div className="mt-4 flex items-start gap-2.5 rounded-xl bg-[#FACE68]/10 ring-1 ring-[#FACE68]/30 px-4 py-3">
              ...
              <p className="text-sm text-[#7A5800]">
                ...
                <strong>{new Date(status.plan_expires_at).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</strong>.
              </p>
            </div>
          )}
        </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-gray-500">${PLAN_PRICING.starter.monthly_cents / 100}/mo · 200 items, Telegram, digest</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-gray-500">${PLAN_PRICING.pro.monthly_cents / 100}/mo · Unlimited, semantic search, briefing</p>
                    </div>
                  </div>
            ...
            {plan === PLAN_STARTER && (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-semibold text-gray-900">Upgrade to Pro</p>
                    <p className="text-xs text-gray-500">Unlimited items, semantic search, daily briefing</p>
                  </div>
                </div>
                <ExternalLink className="h-4 w-4 text-gray-400 group-hover:text-[#FACE68] transition-colors" />
              </button>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-gray-500">Update payment, change plan, view invoices</p>
                  </div>
                </div>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="mx-auto max-w-2xl px-6 py-10">
          ...
          <div className="mb-8 flex items-center gap-4">
            <Link
              href="/settings"
              className="flex h-9 w-9 items-center justify-center rounded-xl bg-white ring-1 ring-gray-100 shadow-sm text-gray-400 hover:text-gray-600 transition-colors"
            >
          ...
          <Suspense fallback={<div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-28 rounded-2xl bg-white ring-1 ring-gray-100 animate-pulse" />)}</div>}>
            <BillingContent />
          </Suspense>
        </main>
      </div>
